...
<head>
                ...
                <div class="card card-block card-stretch">
                    <div class="card-body p-0">
                        <div class="d-flex justify-content-between align-items-center p-3 flex-wrap gap-3">
                            ...
                            @if($auth_user->can('bbn add'))
                            <a href="{{ route('bbn.create') }}" class="float-right mr-1 btn btn-sm btn-primary"><i class="fa fa-plus-circle"></i> {{ __('messages.add_form_title',['form' => __('messages.bbn')  ]) }}</a>
                            @endif
                        </div>
    ...
    <div class="card">
        ...
        <div class="row justify-content-between">
            <div>
                <div class="col-md-12">
                  <form action="{{ route('bbn.bulk-action') }}" id="quick-action-form" class="form-disabled d-flex gap-3 align-items-center">
                    @csrf
                ...
                data--submit="{{ route('bbn.bulk-action') }}"
                data-datatable="reload" data-confirmation='true'
                data-title="{{ __('bbn',['form'=>  __('bbn') ]) }}"
                title="{{ __('bbn',['form'=>  __('bbn') ]) }}"
    ...
    <script>
        ...
        window.renderedDataTable = $('#datatable').DataTable({
                ...
                ajax: {
                  "type"   : "GET",
                  "url"    : '{{ route("bbn.index_data",["list_status" => $list_status]) }}',
                  "data"   : function( d ) {
                    d.search = {
                      value: $('.dt-search').val()
                    };
