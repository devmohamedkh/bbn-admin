<x-master-layout>
    <div class="container-fluid">
        <div class="row">
                ...
                <div class="card card-block card-stretch">
                    <div class="card-body p-0">
                        <div class="d-flex justify-content-between align-items-center p-3 flex-wrap gap-3">
                            ...
                            <a href="{{ route('bbn.index') }}" class="float-right btn btn-sm btn-primary"><i
                                    class="fa fa-angle-double-left"></i> {{ __('messages.back') }}</a>
                            @if($auth_user->can('bbn list'))
                            @endif
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        {{ Form::model($bbndata,['method' => 'POST','route'=>'bbn.store', 'enctype'=>'multipart/form-data', 'data-toggle'=>"validator" ,'id'=>'bbn'] ) }}
                        {{ Form::hidden('id') }}
                        {{ Form::hidden('user_type','bbn') }}
                        <div class="row">
                            ...
                            @if (!isset($bbndata->id) || $bbndata->id == null)
                            <div class="form-group col-md-4">
                            ...
                            <div class="form-group col-md-4">
                                {{ Form::label('bbntype_id', __('messages.select_name',[ 'select' => __('messages.bbntype') ]).' <span class="text-danger">*</span>',['class'=>'form-control-label'],false) }}
                                <br />
                                {{ Form::select('bbntype_id', [optional($bbndata->bbntype)->id => optional($bbndata->bbntype)->name], optional($bbndata->bbntype)->id, [
                                        'class' => 'select2js form-group bbntype',
                                        'required',
                                        'data-placeholder' => __('messages.select_name',[ 'select' => __('messages.bbntype') ]),
                                        'data-ajax--url' => route('ajax-list', ['type' => 'bbntype']),
                                    ]) }}
                            </div>
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <br />
                                {{ Form::select('provider_id', [optional($bbndata->providers)->id => optional($bbndata->providers)->display_name], optional($bbndata->providers)->id, [
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <br />
                                {{ Form::select('country_id', [optional($bbndata->country)->id => optional($bbndata->country)->name], optional($bbndata->country)->id, [
                            ...
                            @if(getMediaFileExit($bbndata, 'profile_image'))
                            <div class="col-md-2 mb-2">
                                <img id="profile_image_preview" src="{{getSingleMedia($bbndata,'profile_image')}}"
                                    alt="#" class="attachment-image mt-1">
                                <a class="text-danger remove-file"
                                    href="{{ route('remove.file', ['id' => $bbndata->id, 'type' => 'profile_image']) }}"
    ...
    (function($) {
        ...
        $(document).ready(function() {
            var country_id = "{{ isset($bbndata->country_id) ? $bbndata->country_id : 0 }}";
            var state_id = "{{ isset($bbndata->state_id) ? $bbndata->state_id : 0 }}";
            var city_id = "{{ isset($bbndata->city_id) ? $bbndata->city_id : 0 }}";

            var provider_id = "{{ isset($bbndata->provider_id) ? $bbndata->provider_id : '' }}";
            var service_address_id =
                "{{ isset($bbndata->service_address_id) ? $bbndata->service_address_id : 0 }}";
